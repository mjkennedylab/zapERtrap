...
methods (Static)
    ...
    function mask = laplacemask(img_in,lsig,gsig)
        if nargin<3
            gsig = ones(1,numel(size(img_in)));
            if nargin<2
                lsig = ones(1,numel(size(img_in)));
        ...
        gch = gaussf(img_in,gsig);
        out = GeneralAnalysis.imgLaplaceCutoff(gch,lsig,gsig);
        thr = multithresh(single(out),3);
        mask = out>thr(1);
    end
    function mask = laplacemask_withinput(img_in,lsig,gsig)
        if nargin<3
            gsig = ones(1,numel(size(img_in)));
            if nargin<2
                lsig = ones(1,numel(size(img_in)));
        ...
        gch = gaussf(img_in,gsig);
        out = GeneralAnalysis.imgLaplaceCutoff(gch,lsig,gsig);
        [mask,threshval,C] = GeneralAnalysis.imgThreshold_fixedUserInput(out);
    end
...
%                 v = dipgetcoords(g,1);
    ...
    function newmask = cleanUpMask(mask_in,imviewsz)
         if nargin<2
             imviewsz = 150;
         end
        lb = label(logical(mask_in));
...
%                 v = dipgetcoords(g,1);
    ...
    function [labeledim] = labelmask_byframe(mask_in,conn,minSize,maxSize)
        ...
        for ii = 1:size(mask_in,3)
            mask = mask_in;
            temp = label(mask(:,:,ii-1),conn,minSize,maxSize);
            temp(temp>0) = ii;
            labeledim(:,:,ii-1) = temp;
            display(['Frame ' num2str(ii) ' of ' num2str(size(mask_in,3))]);
        end
    end
...
%         lbl_out = dip_image(lbl);
    ...
    function lbl_out = keepLables(lbl_in,ids2keep)
        A = single(lbl_in(:));
        B = single(ids2keep);
        idx = ismember(A,B);
        out = A.*idx;
        sizearr = [];
        for ll = 1:length(size(lbl_in))
           sizearr = [sizearr,size(lbl_in,ll)];            
        end
        lbl_out = reshape(out,sizearr);
    end
        
    function measure_structure = measureMaskTimeSeries(labeled,image,measurements)
         if ~isa(image,'dip_image') || ~is(labeled,'dip_image')
            try
                image = dip_image(image);
                labeled = dip_image(labeled,'sint32');
            catch
                warning('input must be an image matrix');
                    return;
            end
        ...
        for tt = 1:maxframe
                currarr = msrarray{tt};
                for m = 1:numel(measurements)
                 measure_structure.(measurements{m})(currarr.ID,tt) = currarr.(measurements{m})';
                end
        end
    end 
